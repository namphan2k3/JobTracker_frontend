...
export function AdminCompanyDetailPage() {
  ...
  const handleVerify = async (isVerified) => {
    ...
    try {
      const data = await verifyCompany(id, { isVerified });
      setCompany(data);
  ...
  return (
    <div className={styles.adminCompanyPage}>
      ...
      <Drawer open={editModalOpen} onClose={() => setEditModalOpen(false)} title="Chỉnh sửa thông tin công ty">
        <form onSubmit={handleSave} className={styles.adminCompanyPage__form}>
          {error && (
            <div className={styles.adminCompanyPage__error} role="alert">{error}</div>
          )}
          ...
          <div className={styles.adminCompanyPage__formActions}>
            <button type="submit" className={styles.adminCompanyPage__submitButton} disabled={saving}>
              {saving ? 'Đang lưu...' : 'Lưu'}
            </button>
